...
import { useCreateAccount, useSetUserActive, useSupervision, useUsers } from '../lib/hooks'
import { usePageTitle } from '../lib/usePageTitle'
import { useToast } from '../lib/toastContext'
import { displayName, isActive, roleLabel, type AppUser, type UserRole } from '../lib/types'
...
export default function UsersPage() {
  ...
  // derived from the query data so the table reacts when links change
  const supervisorNames = useMemo(() => {
    const map = new Map<string, string[]>()
    for (const link of links ?? []) {
      const instructor = (users ?? []).find((user) => user.id === link.instructorId)
      if (!instructor) continue
      map.set(link.taId, [...(map.get(link.taId) ?? []), displayName(instructor)])
    }
  ...
  const visible = useMemo(() => {
    const list = users ?? []
    if (filter === 'instructors') return list.filter((u) => u.role === 'instructor')
    if (filter === 'assistants') return list.filter((u) => u.role === 'teaching_assistant')
    if (filter === 'unassigned') {
      return list.filter(
        (u) =>
          u.role === 'teaching_assistant' &&
          (supervisorNames.get(u.email.toLowerCase()) ?? []).length === 0,
      )
    }
    return list
  ...
  async function onToggleActive(target: AppUser) {
    ...
    try {
      ...
      setRowError({
        id: target.id,
        message: err instanceof Error ? err.message : 'Could not update the account.',
      })
    } finally {
      setPending(null)
    }
  ...
  // deactivating an instructor may strand the people they supervise, so it
  // goes through the decision dialog. Reactivating, and anything on a teaching
  ...
  return (
    <>
        ...
        >
          <label className="flex flex-col gap-2">
            ...
            <input
              ...
              className="h-11 w-48 rounded-md border border-line bg-field px-3.5 text-sm text-ink placeholder:text-hint"
            />
          </label>
          <label className="flex flex-col gap-2">
          ...
          <label className="flex flex-col gap-2">
            ...
            <select
              ...
              className={`h-11 w-44 rounded-md border border-line bg-surface px-3 text-sm ${role ? 'text-ink' : 'text-hint'}`}
            >
          ...
          {role === 'teaching_assistant' && (
            <div className="flex flex-col gap-2">
              <span className="text-xs text-ink-muted">Supervising instructors</span>
              <TokenMultiSelect
                choices={instructors.map((i) => ({ value: i.id, label: displayName(i) }))}
      ...
      <section className="mt-5 rounded-xl border border-line bg-surface px-7 py-2">
        {isPending ? (
          <div className="flex flex-col gap-3 py-5" aria-busy="true" aria-label="Loading users">
            {[0, 1, 2].map((i) => (
              <div key={i} className="h-10 animate-pulse rounded-md bg-navy-50" />
            ))}
          </div>
        ) : (
          // narrow windows scroll the table inside the card rather than spilling out of it
          <div className="overflow-x-auto">
          <table className="w-full min-w-[880px] border-collapse text-left">
            <thead>
              <tr className="border-b border-line">
                {['Name', 'Email', 'Role', 'Supervisors', 'Status', ''].map((h) => (
                  <th
                    key={h}
              ...
              {visible.map((u) => {
                ...
                return (
                  <tr key={u.id} className="border-b border-line last:border-b-0">
                    <td className="py-4 pr-4 text-sm font-medium text-ink">{displayName(u)}</td>
                    <td className="py-4 pr-4 font-mono text-xs text-ink-muted">{u.email}</td>
                    <td className="py-4 pr-4 text-[13px] text-ink-muted">{roleLabel(u)}</td>
                    <td
                    ...
                    <td className="py-4 pr-4">
                      <span
                        className={`rounded-full px-2.5 py-1 text-xs font-medium ${
                          isActive(u) ? 'bg-human-bg text-human' : 'bg-unsure-bg text-unsure'
                        }`}
                      >
                    ...
                    <td className="py-4 text-right whitespace-nowrap">
                      {isSelf ? (
                        <span className="pr-2.5 text-[13px] text-ink-faint">Your account</span>
                      ) : (
                        <>
                          {u.role === 'teaching_assistant' && (
                            <RowAction
                              onClick={() => setRelationship({ subject: u, direction: 'supervisors' })}
                              disabled={busy}
                            >
                              Supervisors
                            </RowAction>
                          ...
                          {u.role === 'instructor' && (
                            <RowAction
                              onClick={() => setRelationship({ subject: u, direction: 'assistants' })}
                              disabled={busy}
                            >
                              Teaching assistants
                            </RowAction>
